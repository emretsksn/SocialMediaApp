...
import com.emretaskesen.tpost.databinding.FragmentTaggedPostBinding
import com.emretaskesen.tpost.model.Post
import com.google.firebase.firestore.FirebaseFirestore
import com.google.firebase.firestore.Query
import timber.log.Timber

class MentionPostFragment : Fragment() {

    private lateinit var binding : FragmentTaggedPostBinding
    ...
    private fun initViewsAndFunctions() {
        ...
        binding.taggedPostDetail.layoutManager = layoutManager
        recyclerViewAdapter = PostAdapter(requireActivity() , postList)
        binding.taggedPostDetail.adapter = recyclerViewAdapter
    }

    @SuppressLint("NotifyDataSetChanged")
    private fun getTagPost(userID : String) {
        db.collection("Post").whereEqualTo("taggedUserID" , userID)
            .orderBy("postDate" , Query.Direction.DESCENDING)
            .addSnapshotListener { snapshot , error ->
                if (error != null) {
                    Toast.makeText(requireActivity(), error.localizedMessage, Toast.LENGTH_LONG).show()
                    error.localizedMessage?.let { Timber.tag("Tag").e(it) }
                }
                else {
                    if (snapshot != null) {
                        if (! snapshot.isEmpty) {
